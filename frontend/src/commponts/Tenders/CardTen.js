...
export default function CardTen({page,item,id,showDelete,sort1}) {
  ...
  // /api/v1/auctions
    ...
    .get(
      ...
      sort=='فرز حسب'||sort==' الوقت'||sort==' مجموعات'
          ? 
          `/api/v1/favorites?type=tender&page=${currentPage}&limit=3`
          : sort == ' جاري' || sort == ' منتهي' || sort == ' قادم'
          ? `/api/v1/favorites?type=tender&activeStatus=${sort.trim()}&page=${currentPage}&limit=3`
          : `/api/v1/favorites?type=tender&categoryName=${sort.trim()}&page=${currentPage}&limit=3`
      }`
      ,
      {
        headers: {
